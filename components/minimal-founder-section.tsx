...
export default function MinimalFounderSection() {
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-center">
            ...
            >
              {/* Floating phrases above the image */}
              <div className="absolute -top-8 left-0 right-0 h-6 overflow-hidden">
                <motion.div
              ...
              {/* Floating phrases below the image */}
              <div className="absolute -bottom-8 left-0 right-0 h-6 overflow-hidden">
                <motion.div
                  className="flex space-x-8 whitespace-nowrap"
                  animate={{
                    x: [-1000, 0],
                ...
                >
                  {phrases.map((phrase, index) => (
                    <span key={`bottom-${index}`} className="text-xs text-gray-400 uppercase tracking-widest">
                      {phrase}
                    </span>
                  ))}
                  {/* Duplicate phrases for seamless loop */}
                  {phrases.map((phrase, index) => (
                    <span key={`bottom-dup-${index}`} className="text-xs text-gray-400 uppercase tracking-widest">
                      {phrase}
                    </span>
                  ))}
                </motion.div>
            ...
            >
              ...
              <div className="space-y-6">
                <motion.div className="relative" animate={textControls} initial={{ opacity: 0, y: 20 }}>
                  <h2 className="text-5xl md:text-6xl lg:text-7xl font-extralight tracking-tighter leading-tight">
